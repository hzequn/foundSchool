        ...
        <div class="rightpart">
            ...
            <form action="" method="get">
                <ul class="sub-search clearfix">
                    ...
                    <input type="hidden" name="opt" id='opt' value="" autocomplete="off">
                    ...
                    <table cellpadding="5" cellspacing="1">
                        ...
                        <tbody>
                            ...
                            <tr {if $smarty.section.key.index % 2==0}class="hui" {/if}>
                                ...
                                <td><a href="l_edit.php?type=1&l_id={$page.list[key].s_id}">{$page.list[key].s_name}</a>
                                </td>
                                <td class="feature">{$page.list[key].s_feature}</td>
                                <td><a href="u_edit.php?id={$page.list[key].s_uid}">{$page.list[key].s_uname}</a></td>
                                <td><a href="l_edit.php?type=0&l_id={$page.list[key].r_id}">{$page.list[key].r_name}</a>
                                </td>
                                <td class="feature">{$page.list[key].r_feature}</td>
                                <td><a href="u_edit.php?id={$page.list[key].r_uid}">{$page.list[key].r_uname}</a></td>
            ...
            <div class="pager" style="margin-bottom:30px;">
                ...
                <a class="{$value.class}"
                    href="?{if $opt!=''}opt={$opt}&{/if}{if $keyword}keyword={$keyword}&{/if}page={$value.pageIndex}">{$value.text}</a>
                {/foreach}
            </div>
            {/if}
        </div>
